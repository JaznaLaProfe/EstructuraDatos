 ...
 * @brief implementar al TAD fila de forma dinámica
 * @version 0.1
 * @date 2025-04-30
 * 
...
void extract(FILA *f){
    if (!empty(*f)){
        /* Guardar nodo a eliminar */
        ELEMENTO temp = (*f).primero;   
        
        /* Avanza */
        (*f).primero = (*f).primero->siguiente; 
        
        /* Verifica si quedó vacía */
        if ((*f).primero == NULL){  
            (*f).ultimo = NULL;
        }
        /* Libera memoria para evitar fuga de memoria */
        free(temp);                    
    }
}
...
void infoFila(FILA f){
    if (f.primero->valor.tipo == TIPO_INT){
        printf("Primero -> %d - Ultimo-> %d\n", f.primero->valor.valor.entero, f.ultimo->valor.valor.entero);
    }
    else{
        printf("Primero -> %s - Ultimo-> %s\n", f.primero->valor.valor.cadena, f.ultimo->valor.valor.cadena);
    }
}
