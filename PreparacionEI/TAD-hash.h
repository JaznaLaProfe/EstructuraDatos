...
int hash(int, int);
void verTablaHash(HASH, char*);
HASH crearTabla(int);
HASH agregarClave(HASH, int);
int buscarHash(HASH, int);
 ...
 */
void verTablaHash(HASH tabla, char *nombre_archivo){
	int indice;
	ELEMENTO_H auxiliar;
    FILE *archivo_hash = fopen(nombre_archivo, "w");
...
HASH agregarClave(HASH tabla, int valor){
	...
	ELEMENTO_H nuevo = (ELEMENTO_H) malloc(sizeof(struct s_lista));
	nuevo->valor = valor;
 ...
 */
int buscarHash(HASH tabla, int target){	
	ELEMENTO_H auxiliar;
